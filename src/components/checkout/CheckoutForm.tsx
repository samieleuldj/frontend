...
export default function CheckoutForm({ productName, price }: CheckoutFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // إرسال البيانات إلى جوجل شيت (Webhook)
      ...
      if (webhookUrl && webhookUrl !== 'your_google_script_url_here') {
        await fetch(webhookUrl, {
          method: 'POST',
          mode: 'no-cors', // مهم جداً لجوجل شيت لتجنب أخطاء CORS
          headers: {
        ...
        console.log('Google Sheet Webhook URL is not configured. Order data:', orderData);
      }
    } catch (error) {
      console.error('Error saving order to Google Sheets:', error);
      // نستمر في العملية حتى لو فشل الحفظ في الشيت لكي لا نخسر الزبون
    }
